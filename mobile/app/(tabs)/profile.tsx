...
export default function ProfileScreen() {
  ...
  const menuItems = [
      ...
      { label: 'Log Out', icon: 'log-out-outline', route: '/login', color: 'text-red-500', iconColor: '#EF4444' },
  ];
    ...
    <ScreenWrapper bg="bg-[#FAFAFA]">
      <ScrollView className="flex-1 px-6 pt-6" contentContainerStyle={{ paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
         ...
         <View className="flex-row items-center mb-8 bg-white p-6 rounded-[32px] shadow-sm shadow-black/5 border border-gray-100">
             <View className="w-20 h-20 bg-emerald-50 rounded-full items-center justify-center mr-5 border-2 border-white shadow-sm overflow-hidden">
                 <Image 
             ...
             <View>
                 <ThemedText variant="h2" weight="bold" className="text-xl text-gray-900 mb-1">Sarah Green</ThemedText>
                 <ThemedText color="gray" className="text-sm opacity-60 mb-2">sarah.green@example.com</ThemedText>
                 <View className="bg-emerald-100 px-3 py-1 rounded-full self-start">
                     <ThemedText color="primary" weight="bold" className="text-xs text-[#2D6A4F]">Gold Member</ThemedText>
                 </View>
             </View>
         </View>
         ...
         <View className="bg-white rounded-[32px] shadow-sm shadow-black/5 border border-gray-100 overflow-hidden mb-8">
             {menuItems.map((item, index) => (
                 ...
                 >
                     <View className={`w-12 h-12 rounded-2xl items-center justify-center mr-4 ${item.color ? 'bg-red-50' : 'bg-gray-50'}`}>
